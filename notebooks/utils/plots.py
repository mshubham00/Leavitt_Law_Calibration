...
def plottransformation(data, raw, w, dis, c1 = 'JH', c2 = 'HK', title = 'magnitude_comparision', s=0):
    plt.figure(figsize=(6,6))
    data['logP'] = data['logP']-1
    raw['logP'] = raw['logP']-1
    for i in range(0,1):
        jh = pltfill(data['logP'], w[mag[0]+c1+dis],w[mag[5]+c1+dis],'g')
        hk = pltfill(data['logP'], w[mag[0]+c2+dis],w[mag[5]+c2+dis],'c')
        tabs = pltfill(data['logP'], data[bands[0]+'0'+dis],data[bands[5]+'0'+dis],'y')
        ra = pltfill(raw['logP'], raw[ap_bands[0]],raw[ap_bands[5]],'r')
        abs = pltfill(raw['logP'], data[bands[0]+'0'+dis] + data['plx'],data[bands[5]+'0'+dis] + data['plx'],'m')
        plt.text(0, -10.4, f'Wesenheit Magnitude ({c1})', color='g')
        plt.text(0.52, -7.0, f'({c2})', color='k')
        plt.text(0, -2.2, f'True Absolute Magnitude',color='grey')
        plt.text(0, 14.2, f'Apparent Magnitude',color='red')  
        plt.text(0, 2.5, f'True Apparent Magnitude',color='m')
