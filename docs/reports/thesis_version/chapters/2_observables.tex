...
The Gaia satellite, launched by the European Space Agency in 2013, has revolutionized our understanding of the Milky Way by measuring precise the parallax and proper motion of billions of objects within the Galaxy and beyond. Among the vast number of objects observed, several notable types of variable stars, including Cepheid variables and RR Lyrae stars, were also studied. In this thesis, the parallax-driven distances provided by Gaia will be used to calibrate the period-luminosity relation. 

Since Cepheid variable are bright yellow giants, their high intensity of photons saturates the sensitive pixels of CCD camera resulting some systematics in parallax measurement. Hence, parallax based distances of Cepheids can have marginal errors which I would be determining in this thesis. 


\\subsection{Infrared Surface Brightness Method}

This method relies on a simple analogy that larger object covers larger area on the sky. If larger object covering smaller area of sky, that means it is located far away. Mathematically, it can be written as: 

$$Distance, d = \frac{Physical diameter}{Angular Diameter}$$

The angular diameter of Cepheid is determined from the infrared color (V-K), which corresponds to the surface temperature and further can be transformed into luminosity using stefan-boltzmann law. Using emperical correlation formula, color can be related to surface brightness as:

$$S_V = a (V-K) + b$$

where $S_V$ represents the surface brightness in V-band. 

And the empirical relation between absolute luminosity, surface brighness and angular diameter (in mas) is given as:

$$S_V = V_0 + 5 log \theta$$

Both together leads to 
$$\theta (\phi) = 10^{0.2[a(V-K)_0(\phi) + b - V_0(\phi)]}$$

To estimate physical diameter, first radial velocity is measured using spectroscopic observation (dopper effect). The radial velocity is then converted into photospheric pulsation velocity by calibrating for projection factor (p-factor). The projection factor includes the effect of geometrical projection as central region moves along the line-of-sight but not that not the case around the edges. Another effect is limb darkening as centeral region appears much brighter than surrounding disk affecting the contribution of spectral lines. It is also important to acknowledge that the different spectral lines forms at different depth which moving with different velocities. And over the pulsation cycle, optical depth also varies with the temperature shifting the photosphere over the different layer of gases. The product of these factors altogether yields the p-factor. Upon calibration for p-factor, the pulsation velocity can be written as:   

$v_p(t) = p[v_r(t) - v_{\gamma}]$

where $v_p$ is pulsation velocity, $v_r$ is radial velocity, $p$ is projection factor and $v_{\gamma}$ is mean velocity. Integrating the pulsation velocity of the Cepheid for the cycle gives the change in radius.

$$\Delta R = \int{t_1}{t_0} v_p(t)dt$$

Instead of time, pulsation phase is used as the system is periodic. 

$$\phi = \frac{t-t_0}{P}   \implies dt = P d\phi$$

$$\Delta R = pP \int{\phi}{0}[v_r(\phi') - v_{\gamma}]d\phi '$$

Hence, angular radius at any phase given as:

$$R(\phi) = R_0 + \Delta R(\phi)$$

Substituting angular diameter and physical diameter in the first relation:

$$\theta (\phi) = \frac{2R_0}{d} + \frac{2pP}{d} \int{\phi}{0}(v_r - v_{\gamma})d\phi $$

which can be compared with linear equation form:

$$\theta (\phi) = A + B X(\phi)$$
here $\theta (\phi)$ and $X (\phi)$ are deduced from the observation leading to the values of intercept A and slope B

The value of slope B leads to the measurment of distance. 
