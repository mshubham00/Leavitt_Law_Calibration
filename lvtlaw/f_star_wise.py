...
#            star['abs'+dis] = data[['M_%s%s'%(x, dis) for x in mag]].iloc[i].values
            ...
            for c in wes_show:
                ...
                for f in flags:
                    star['d_%s%s%s'%(f,c,dis)] = data[['d_%s%s%s%s'%(x,x if f=='S' else c[0],c,dis) for x in mag]].iloc[i].values
...
def star_ex_rd_mu(mu_df_list_dict, data, flags=flags, dis_flag=dis_flag, p=1):
    ...
    for i in range(0, n):
        ...
        for c in wes_show:
            for dis in dis_flag:
                for ab in mode:
                    colms = [f'rd{ab}_{c}{mu}{dis}' for mu in del_mu]
                    for f in flags:
                        rdMS = pd.DataFrame()
                        for m in range(len(mag)):
                            rdMS[mag[m]] = mu_df_list_dict[f'{c}_{f}{dis}'][m][colms].iloc[i].values
                        rdMS = rdMS.T
                        rdMS.columns = [f'{f}{ab}{c}{dis}rd_{mu}' for mu in del_mu]
