...
#from lvtlaw.visualization import cordinate
from lvtlaw.utils import output_directories, load_data, process_step, colors,data_dir, data_file, data_out, dis_list
...
dres_S, dpre_S, del_mc_S, dres_M, dpre_M, del_mc_M = residue_analysis(residue, dis = ['_g','_i'], cols = ['BV', 'VI','VK','JK'],s=1)
...
for flag in ['_S', '_M']:
    if flag == '_S':
        ...
        dresidue = dres_M
        slope = del_mc_M
    lists = result( absolute, residue, slope, dis, col, flag)
    method.append(lists)
...
print('Enter to open the output folder!!')
open_folder(data_out)
